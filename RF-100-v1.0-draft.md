...
**Patent/IPR Position.** Remnant Fieldworks supports open implementation and independent assessment of RF-100. A **draft IPR commitment** describing the intended patent posture — a royalty-free, scoped, reciprocal (defensive) non-assertion covenant for conforming implementations, with rights reserved for trademarks, products, hosted services, reference code, and the certification program — is published alongside this draft as `IPR-COMMITMENT.md` in the RF-100 repository. That document (v0.2-draft) has been revised to incorporate a pre-counsel legal review — adding successors/assigns and runs-with-the-patents language, a version-lock irrevocability rule, third-party-beneficiary intent, defined terms, worldwide scope, an Ohio governing-law clause, and a no-implied-rights clause — but remains a **draft pending formal legal/patent counsel review** (see its Annex IPR-A for open action items). The binding version will be finalized and published before any formal certification or conformance program (RF-101 / RF-102) is launched.
